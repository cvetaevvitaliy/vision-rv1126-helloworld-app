...
static pthread_mutex_t aiq_ctx_mutex[MAX_AIQ_CTX] = { PTHREAD_MUTEX_INITIALIZER, PTHREAD_MUTEX_INITIALIZER,
                                                      PTHREAD_MUTEX_INITIALIZER, PTHREAD_MUTEX_INITIALIZER };
pthread_mutex_t lock[MAX_AIQ_CTX] = { PTHREAD_MUTEX_INITIALIZER, PTHREAD_MUTEX_INITIALIZER, PTHREAD_MUTEX_INITIALIZER,
...
rk_aiq_working_mode_t g_wdr_mode[MAX_AIQ_CTX];

typedef enum SHUTTERSPEED_TYPE_E {
    SHUTTERSPEED_1_25 = 0,
    SHUTTERSPEED_1_30,
    SHUTTERSPEED_1_75,
    SHUTTERSPEED_1_100,
...
static SHUTTER_ATTR_S g_st_shutter_attr[SHUTTERSPEED_BUTT] = {
...
RK_S32 SAMPLE_COMM_ISP_Init(RK_S32 CamId, rk_aiq_working_mode_t WDRMode, RK_BOOL MultiCam, const char* iq_file_dir)
{
    ...
    if (rk_aiq_uapi_sysctl_enumStaticMetas(CamId, &aiq_static_info) != 0) {
        return -1;
    }
...
RK_S32 SAMPLE_COMM_ISP_SetFecEn(RK_S32 CamId, RK_BOOL bFECEnable)
{
    ...
    return rk_aiq_uapi_setFecEn(g_aiq_ctx[CamId], bFECEnable);
}
...
RK_S32 SAMPLE_COMM_ISP_Run(RK_S32 CamId)
{
    ...
    if (rk_aiq_uapi_sysctl_prepare(g_aiq_ctx[CamId], 0, 0, g_wdr_mode[CamId])) {
...
RK_S32 SAMPLE_COMM_ISP_DumpExpInfo(RK_S32 CamId, rk_aiq_working_mode_t WDRMode)
{
    ...
    char a_str[128] = { '\0' };
    Uapi_ExpQueryInfo_t st_exp_info;
    rk_aiq_wb_cct_t st_cct;
    RK_S32 ret = 0;
    ret = rk_aiq_user_api_ae_queryExpResInfo(g_aiq_ctx[CamId], &st_exp_info);
    ret |= rk_aiq_user_api_awb_GetCCT(g_aiq_ctx[CamId], &st_cct);

    if (WDRMode == RK_AIQ_WORKING_MODE_NORMAL) {
        sprintf(a_str, "M:%.0f-%.1f LM:%.1f CT:%.1f",
                st_exp_info.CurExpInfo.LinearExp.exp_real_params.integration_time * 1000 * 1000,
                st_exp_info.CurExpInfo.LinearExp.exp_real_params.analog_gain, st_exp_info.MeanLuma, st_cct.CCT);
    } else {
        sprintf(a_str,
                "S:%.0f-%.1f M:%.0f-%.1f L:%.0f-%.1f SLM:%.1f MLM:%.1f "
                "LLM:%.1f CT:%.1f",
                st_exp_info.CurExpInfo.HdrExp[0].exp_real_params.integration_time * 1000 * 1000,
                st_exp_info.CurExpInfo.HdrExp[0].exp_real_params.analog_gain,
                st_exp_info.CurExpInfo.HdrExp[1].exp_real_params.integration_time * 1000 * 1000,
                st_exp_info.CurExpInfo.HdrExp[1].exp_real_params.analog_gain,
                st_exp_info.CurExpInfo.HdrExp[2].exp_real_params.integration_time * 1000 * 1000,
                st_exp_info.CurExpInfo.HdrExp[2].exp_real_params.analog_gain, st_exp_info.HdrMeanLuma[0],
                st_exp_info.HdrMeanLuma[1], st_exp_info.HdrMeanLuma[2], st_cct.CCT);
    }
...
RK_S32 SAMPLE_COMM_ISP_SET_ManualExposureAutoGain(RK_S32 CamId, RK_U32 u32Shutter)
{
    ...
    RK_S32 ret = 0;
    Uapi_ExpSwAttr_t st_exp_sw_attr;
    pthread_mutex_lock(&aiq_ctx_mutex[CamId]);
    if (g_aiq_ctx[CamId]) {
        ret = rk_aiq_user_api_ae_getExpSwAttr(g_aiq_ctx[CamId], &st_exp_sw_attr);
        st_exp_sw_attr.AecOpType = RK_AIQ_OP_MODE_MANUAL;
        st_exp_sw_attr.stManual.stLinMe.ManualGainEn = RK_FALSE;
        st_exp_sw_attr.stManual.stLinMe.ManualTimeEn = RK_TRUE;
        st_exp_sw_attr.stManual.stLinMe.TimeValue = g_st_shutter_attr[u32Shutter % SHUTTERSPEED_BUTT].fExposureTime;

        st_exp_sw_attr.stManual.stHdrMe.ManualGainEn = RK_FALSE;
        st_exp_sw_attr.stManual.stHdrMe.ManualTimeEn = RK_TRUE;
        st_exp_sw_attr.stManual.stHdrMe.TimeValue.fCoeff[0] =
            g_st_shutter_attr[u32Shutter % SHUTTERSPEED_BUTT].fExposureTime;
        st_exp_sw_attr.stManual.stHdrMe.TimeValue.fCoeff[1] =
            g_st_shutter_attr[u32Shutter % SHUTTERSPEED_BUTT].fExposureTime;
        st_exp_sw_attr.stManual.stHdrMe.TimeValue.fCoeff[2] =
            g_st_shutter_attr[u32Shutter % SHUTTERSPEED_BUTT].fExposureTime;
        ret |= rk_aiq_user_api_ae_setExpSwAttr(g_aiq_ctx[CamId], st_exp_sw_attr);
    }
    pthread_mutex_unlock(&aiq_ctx_mutex[CamId]);
    return ret;
}
...
RK_S32 SAMPLE_COMM_ISP_SET_ManualExposureManualGain(RK_S32 CamId, RK_U32 u32Shutter, RK_U32 u32Gain)
{
    ...
    if (g_aiq_ctx[CamId]) {
        ret = rk_aiq_user_api_ae_getExpSwAttr(g_aiq_ctx[CamId], &st_exp_sw_attr);
        st_exp_sw_attr.AecOpType = RK_AIQ_OP_MODE_MANUAL;
        st_exp_sw_attr.stManual.stLinMe.ManualGainEn = RK_TRUE;
        st_exp_sw_attr.stManual.stLinMe.ManualTimeEn = RK_TRUE;
        st_exp_sw_attr.stManual.stLinMe.TimeValue = g_st_shutter_attr[u32Shutter % SHUTTERSPEED_BUTT].fExposureTime;
        //(1+gain)*4;//gain[0~255] -> GainValue[1~1024]
        st_exp_sw_attr.stManual.stLinMe.GainValue = (1.0 + u32Gain);

        st_exp_sw_attr.stManual.stHdrMe.ManualGainEn = RK_TRUE;
        st_exp_sw_attr.stManual.stHdrMe.ManualTimeEn = RK_TRUE;
        st_exp_sw_attr.stManual.stHdrMe.TimeValue.fCoeff[0] =
            g_st_shutter_attr[u32Shutter % SHUTTERSPEED_BUTT].fExposureTime;
        st_exp_sw_attr.stManual.stHdrMe.TimeValue.fCoeff[1] =
        ...
        st_exp_sw_attr.stManual.stHdrMe.TimeValue.fCoeff[2] =
            g_st_shutter_attr[u32Shutter % SHUTTERSPEED_BUTT].fExposureTime;
        //(1+gain)*4;//gain[0~255] -> GainValue[1~1024]
        st_exp_sw_attr.stManual.stHdrMe.GainValue.fCoeff[0] = (1.0 + u32Gain);
        //(1+gain)*4;//gain[0~255] -> GainValue[1~1024]
        st_exp_sw_attr.stManual.stHdrMe.GainValue.fCoeff[1] = (1.0 + u32Gain);
        //(1+gain)*4;//gain[0~255] -> GainValue[1~1024]
        st_exp_sw_attr.stManual.stHdrMe.GainValue.fCoeff[2] = (1.0 + u32Gain);
        ret |= rk_aiq_user_api_ae_setExpSwAttr(g_aiq_ctx[CamId], st_exp_sw_attr);
    }
    pthread_mutex_unlock(&aiq_ctx_mutex[CamId]);
    return ret;
...
RK_S32 SAMPLE_COMM_ISP_SET_AutoExposure(RK_S32 CamId)
{
    ...
    if (g_aiq_ctx[CamId]) {
        ret = rk_aiq_user_api_ae_getExpSwAttr(g_aiq_ctx[CamId], &st_exp_sw_attr);
        st_exp_sw_attr.AecOpType = RK_AIQ_OP_MODE_AUTO;
        st_exp_sw_attr.AecRunInterval = 0;
        ret |= rk_aiq_user_api_ae_setExpSwAttr(g_aiq_ctx[CamId], st_exp_sw_attr);
    }
    pthread_mutex_unlock(&aiq_ctx_mutex[CamId]);
    return ret;
                                    ...
                                    RK_U32 u32Agc)
{
    ...
    if (!bIsAutoExposure) {
        if (bIsAGC) {
            ...
            ret = SAMPLE_COMM_ISP_SET_ManualExposureManualGain(CamId, u32ElectronicShutter, u32Agc);
        }
    } else {
        ...
        if (bEnable) {
            ...
            if (ret == 0) {
                ...
                ret = rk_aiq_uapi_setHLCStrength(g_aiq_ctx[CamId], u8Strength / 2.55) ||
                      rk_aiq_uapi_setDarkAreaBoostStrth(g_aiq_ctx[CamId], u8Level / 25);
            }
        } else {
...
RK_S32 SAMPLE_COMMON_ISP_SET_ManualWhiteBalance(RK_S32 CamId, RK_U32 u32RGain, RK_U32 u32GGain, RK_U32 u32BGain)
{
    ...
    printf("auto gain r g g b %f, %f, %f, %f\n", gs_wb_auto_gain.rgain, gs_wb_auto_gain.grgain, gs_wb_auto_gain.gbgain,
           gs_wb_auto_gain.bgain);
    pthread_mutex_lock(&aiq_ctx_mutex[CamId]);
    if (g_aiq_ctx[CamId]) {
        ret = rk_aiq_uapi_setWBMode(g_aiq_ctx[CamId], OP_MANUAL);
...
RK_S32 SAMPLE_COMM_ISP_SET_Correction(RK_S32 CamId, RK_U32 u32Mode, RK_U32 u32Value)
{
    ...
    if (g_aiq_ctx[CamId]) {
        ...
        if (u32Mode) {
            ret |= rk_aiq_uapi_setLdchCorrectLevel(g_aiq_ctx[CamId], (u32Value < 2 ? 2 : u32Value));
        }
    }
    pthread_mutex_unlock(&aiq_ctx_mutex[CamId]);
    return ret;
